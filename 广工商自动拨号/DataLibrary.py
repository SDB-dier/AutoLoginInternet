...
def 联网测试():
    ...
    测试结果 = [0, 0, 0]
    设置文档 = UD.读取数据(GUI.共享设置路径)
    # 首先测试能否连通登录页，如果登录页都无法连通，则说明无法进行登录，直接返回测试结果
    try:
        登录页测试 = requests.get("http://172.30.100.2")
        if 登录页测试.status_code == 200:
            测试结果[0] = 1
    except Exception as e:
        print(datetime.now(), "e", e)
        return 测试结果
    ...
    for i in 抽取结果:
        ...
        try:
            ...
            print(datetime.now(), "测试网站:", i, "测试问题:", e)
            pass
    return 测试结果
...
class 自动复拨(threading.Thread):
    ...
    def run(self):
        ...
        if self.是否运行 is False:
            self.是否运行 = True
            判断拨号线程 = threading.Thread(
                target=self.判断拨号, daemon=True, name="判断拨号线程")
            判断拨号线程.start()
            time.sleep(1)
            # 暂停1秒的作用是使判断拨号函数先修改 是否拨号 变量
            自动拨号线程 = threading.Thread(
                target=self.自动拨号, daemon=True, name="自动拨号线程")
            自动拨号线程.start()

    def 判断拨号(self):
        # 函数作用：判断当前时间是否需要拨号，如果不需要则使 是否拨号 变量为否，使其终止自动复拨
        while 1:
            print(datetime.now(), "DL自动复拨CL判断拨号", "判断拨号,是否终止:", self.是否终止)
            while self.是否终止 == "0":
                时间 = datetime.now()
                今天 = 时间.date()
                今天星期几 = 时间.weekday()
                ...
                if self.开始时间 <= 现在时间 <= self.结束时间 or \
                    ...
                    self.是否拨号 = False
                print(datetime.now(), "DL自动复拨CL判断拨号", "判断拨号是否拨号", self.是否拨号)
    ...
    def 自动拨号(self):
        global 进程ID
        进程ID = threading.get_ident()
        print("自动复拨,进程ID", 进程ID)
        print("自动复拨,所有进程", threading.enumerate())
        # 函数作用：根据 是否拨号 变量决定是否进行复拨
        while 1:
            print(datetime.now(), "DL自动复拨CL自动拨号", "自动拨号", self.是否终止)
            while self.是否终止 == "0":
                print(datetime.now(), "DL自动复拨CL自动拨号", "自动拨号while", self.是否拨号)
                while self.是否拨号:
                    print(datetime.now(), "DL自动复拨CL自动拨号",
                          "正在自动拨号，self.联网失败次数", self.联网失败次数)
                    ...
                    try:
                        ...
                        if (self.联网失败次数 >= 5):
                            ...
                            try:
                                ...
                                break
                    print(datetime.now(), "DL自动复拨CL自动拨号",
                          "自动拨号whilewhile是否拨号", self.是否拨号)
                self.是否终止 = 自动复拨.读取终止(self)
                print(datetime.now(), "DL自动复拨CL自动拨号",
                      "自动拨号while.time.sleep")
